...
int main(int argc, char *argv[])
{
	...
	server_addr.sin_addr.s_addr = inet_addr("127.0.0.1");
    	...
    	int flag = connect(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr));
    	...
    	/*int chars = atoi(buffer);
    	readbytes = read(sockfd, buffer, sizeof(buffer));
    	int words = atoi(buffer);
    	readbytes = read(sockfd, buffer, sizeof(buffer));
    	int sentences = atoi(buffer);*/
